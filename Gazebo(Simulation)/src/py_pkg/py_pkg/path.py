...
class GoalToPathNode(Node):
    def __init__(self) -> None:
        ...
        # Nghĩa là vật cản bị dilate/phình ra bằng kernel 15×15 cell .

        # Nếu map resolution là 0.05 m/cell:b15 cells × 0.05 m = 0.75 m

        # Nhưng lưu ý: kernel 15×15 không có nghĩa vật cản nở đều 15 cell mỗi bên. Với dilation kernel 15, nó thường nở khoảng 7 cell mỗi phía tính từ tâm kernel.
        self.declare_parameter("cspace_padding", 8)


        # → kiểm tra hai bên đường đi khoảng 2 ô
        # → nếu vật cản lọt vào vùng này thì coi như đường không còn an toàn thi replan
        self.declare_parameter("path_corridor_half_width_cells", 2)
        
        # Cái này quyết định giá trị map bao nhiêu thì bị coi là blocked / obstacle.
    ...
    def publish_empty_path(self, reason: str = "unknown") -> None:
        empty_path = Path()
        ...
        self.latest_path_msg = empty_path
        self.latest_path_cells = []

        self.get_logger().warn(
            f"[{reason}] Published EMPTY path to {self.path_topic}."
        )
    ...
    def plan_to_goal(
        ...
        try:
            ...
            self.publish_empty_path(reason=f"{reason}_astar_failed")
            self.latest_goal_msg = goal_msg
            return
        ...
        if not path_cells:
            ...
            self.publish_empty_path(reason=reason)

            # Vẫn giữ goal mới nhất để sau này replan còn biết goal là gì
            self.latest_goal_msg = goal_msg
