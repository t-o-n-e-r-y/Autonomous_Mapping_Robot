...
class FrontendCore:
    ...
    #Submap được tạo khi robot ở odom (2.0, 1.0, 0)
    # Sau đó robot ở odom (2.3, 1.0, 0) Thì trong submap: x = 0.3m y = 0.0m yaw = 0
    ...
    # Nếu pose được tính trong previous_submap,
    # chuyển nó sang hệ tọa độ active_submap. 
    ...
    # Quyết định có đóng active_submap và tạo submap mới không.

    # Điều kiện:
    # 1. Chưa đủ min scan -> không rollover
    # 2. Đủ max scan -> rollover
    # 3. Đi xa hơn threshold -> rollover
    # 4. Quay hơn threshold -> rollover
    ...
    # Tính xem scan hiện tại khớp submap ở pose này tốt cỡ nào.

    # Luồng:

    # 1. Đảm bảo distance field đã có
    # 2. Tính sensor_pose = base_pose_in_submap * base_to_scan_pose
    # 3. Lấy các range hợp lệ
    # 4. Lấy mỗi N tia theo scan_match_stride
    # 5. Chuyển điểm hit từ laser frame sang submap frame
    # 6. Đổi hit point sang grid cell
    # 7. Lấy distance tới vật cản gần nhất
    # 8. Tính likelihood
    # 9. Return mean likelihood
    ...
    # Mục đích:
    # Tìm pose quanh guess_pose sao cho scan khớp submap tốt nhất.
    # Đầu tiên nó tính score tại guess:
    # guess_score = compute_score(...)

    # Nếu chưa đủ điều kiện matching:
    # not enable_local_matching
    # or scan_count < min_submap_scans_before_matching
    # or known_cells < min_known_cells_before_matching

    # thì trả luôn guess_pose.

    # Nếu được match, nó chạy 2 vòng

    # Coarse search

    # match_linear_window_m
    # match_angular_window_deg
    # match_coarse_linear_step_m
    # match_coarse_angular_step_deg

    # Nó thử nhiều candidate quanh guess.

    # Fine search

    # match_coarse_linear_step_m làm window
    # match_coarse_angular_step_deg làm angular window
    # match_fine_linear_step_m
    # match_fine_angular_step_deg

    # Tức fine search tìm quanh kết quả coarse.

    # Sau đó kiểm tra:

    # if fine_score < guess_score + match_accept_min_score_delta:
    #     return guess_pose
    # else:
    #     return fine_pose

    # Tức nếu scan matching không cải thiện đủ, không nhận.
    #     guess_score = compute_score(...)
    ...
    #  Mục đích:
    # Quyết định đã đủ dữ liệu để bắt đầu kiểm tra tracking nghiêm túc chưa.
    # Nếu chưa đủ scan hoặc known cells:
    # return False
    ...
    # Mục đích:
    # Kiểm tra candidate pose từ scan matching có đáng tin không.
    # Nó tính correction giữa odom guess và candidate:
    # correction = relative_pose(odom_guess, candidate_pose)
    # corr_trans = hypot(correction.x, correction.y)
    # corr_rot_deg = abs(degrees(correction.yaw))
...
class SlamSubmapGraphNode(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter("map_publish_rate_hz", 0.2)
        self.declare_parameter("active_submap_publish_rate_hz", 0.4)
